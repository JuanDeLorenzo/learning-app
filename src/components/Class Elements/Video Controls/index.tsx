...
const Controller = ({selected = 'notes', ...rest}:ControllerProps) => {
    return <div className="w-full h-[30.29px] p-0.5 bg-secondary-500 rounded-[9px] shadow justify-center items-center gap-0.5 inline-flex">
        {selected === 'notes' ?
            <Button variant={'video'} disabled>Notes</Button>
            :
            <Button variant={'video'}>Notes</Button>
        }
        <div className="w-px h-3 opacity-30 bg-greyscale-400 rounded-[0.50px]"></div>
